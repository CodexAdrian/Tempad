...
public class ColorSelectScreen extends BaseTempadScreen2 {
    ...
    protected void init() {
        ...
        for (int i = 0; i < colors.length; i++) {
            ...
            //Set it's position
            int finalI = i;
            SpruceColorButton button = new SpruceColorButton(startX + x, startY + y, colors[i], scale * 2, scale * 2, button1 -> {
                FriendlyByteBuf buf = new FriendlyByteBuf(Unpooled.buffer());
                buf.writeInt(colors[finalI]);
                this.color = colors[finalI];
                //Minecraft.getInstance().setScreen(null);
                Minecraft.getInstance().setScreen(new ColorSelectScreen(colors[finalI], this.player, this.hand));
